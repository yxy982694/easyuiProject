		...
		ajaxUrl: function(op) {
			...
			$.ajax({
				...
				success: function(response,status,jqXHR) {
					var re = jqXHR.getResponseHeader('REDIRECT');
					if (re==='REDIRECT' || re === 'redirect') {
						window.location.href = '/haeoms/toLogin';
						return;
					}
...
(function($) {
	if ($.validator) {
		...
		$.validator.addMethod("phone", function(value, element) {
			return this.optional(element) || /^([0-9 \(\)]{7,30})|([0-9]{3,5}-[0-9]{7,8})$/.test(value);
...
(function($) {
	$.fn.jDrag = function(options) {
		...
		return this.each(function() {
			...
			if (options.event) $.rwdrag.start(options.event);
			else {
	...
	$.rwdrag = {
		...
		drag: function(e) {
			...
			if (data.options.move == 'horizontal') {
				if ((data.options.minW && left >= $(data.options.obj).cssv("left") + data.options.minW) && (data.options.maxW && left <= $(data.options.obj).cssv("left") + data.options.maxW)) current.el.style.left = left + 'px';
					...
					} else current.el.style.left = left + 'px';
				}
...
function initLayout() {
  ...
  if ($("#container .tabsPageContent").height(iContentH - 34).find("[layoutH]")) {
...
function initUI(_box) {
	...
	if ($.fn.datepicker) {
		$('input.date', $p).each(function() {
			...
			if ($this.attr("openChange")) opts.openChange = $this.attr("openChange");
	...
	$("a[target=navTabb]", $p).each(function() {
		$(this).click(function(event) {
			var $this = $(this);
			var title = $this.attr("title") || $this.text();
			var tabid = $this.attr("rel") || "_blank";
			...
			$.ajax({
				...
				success:function(data,status,jqXHR){ //请求成功后处理函数。
					var re = jqXHR.getResponseHeader('REDIRECT');
					if (re==='REDIRECT' || re === 'redirect') {
						window.location.href = '/haeoms/toLogin';
						return;
	...
	if ($.fn.pagerForm) $("form[rel=pagerForm]", $p).pagerForm({
		parentBox: $p
	});
}(function($) {
	$.fn.extend({
...
var navTab = {
	...
	_closeTab: function(index, openTabid) {
		...
		//解决关闭嵌入有iframe导致的其他页面焦点丢失的问题输入框不能编辑
            ...
            if (jDContent.find("[layoutH]")) {
              jDContent.find("[layoutH]").layoutH(jDContent);
				...
				$("div.dialogHeader", dialog).dblclick(function() {
					if ($("a.restore", dialog).is(":hidden")) $("a.maximize", dialog).trigger("click");
					else $("a.restore", dialog).trigger("click");
				});
				if (op.max) {
					$.pdialog.maxsize(dialog);
					dialog.jresize("destroy").dialogDrag("destroy");
...
function dwzPageBreak(options) {
	...
	if (op.rel) {
		...
		if (op.targetType == "dialog") {
			if (form) $.pdialog.reload($(form).attr("action"), {
				data: params,
				callback: op.callback
			});
		} else {
			if (form) navTab.reload($(form).attr("action"), {
				data: params,
				callback: op.callback
			});
		}
...
(function($) {
	$.fn.extend({
		...
		pagerForm: function(options) {
			...
			return this.each(function() {
				var $searchForm = $(this),
					$pagerForm = $(op.pagerForm$, op.parentBox);
	...
	$.extend({
		bringBackSuggest: function(args) {
			...
			$box.find(":input").each(function() {
				var $input = $(this),
					inputName = $input.attr("bringBackName");
				for (var key in args) {
...
(function($) {
	...
	$.fn.datepicker = function(opts) {
		...
		return this.each(function() {
			...
			var openChange = dp.get('openChange');
	...
	$.extend($.fn, {
		jPanel: function(options) {
			...
			return this.each(function() {
				...
				if (close) $content.css({
					height: "0px",
					display: "none"
				});
				else {
					if (defaultH > 0) $content.height(defaultH + "px");
					else if (minH > 0) {
				...
				if (ref && refUrl) {
					function _onchange(event) {
						...
						$.ajax({
							type: 'POST',
							dataType: "json",
							url: refUrl.replace("{value}", $this.attr("value")),
							cache: false,
							data: {
...
(function($) {
    ...
    if ($printBox.html($contentBox.html()).find("[layoutH]")) {
